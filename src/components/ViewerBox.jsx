...
import { renderFusion, getSlicePixelData, buildLUT } from '../utils/canvasFusion.js';
...
const VP_BACKGROUND = { CT: [0,0,0], PET: [0,0,0], MIP: [1,1,1] };
...
export default function ViewerBox({
  ...
  // MIP rotation is started directly in setup() after applyMIPVolume completes.
  // No poll needed — setup() has direct access to the viewport and camera
  // at the exact moment the volume is loaded and before any render resets values.
    ...
    async function setup() {
      ...
      if (isVolume) {
        ...
        if (modality === 'CT') {
          ...
          try {
            const _c = vp.getCamera();
            if (_c) {
              // parallelScale: zoom factor — read from applyMIPVolume's pre-render capture
              const scale = _c.parallelScale || window.__mipScale || 1517;
              window.__mipScale = scale;
              // focalPoint and position: read now before any further renders
              const fp  = _c.focalPoint || [0, 0, 0];
              const pos = _c.position   || [0, 0, 0];
              mipFPRef.current   = [...fp];
              const actualDist   = Math.hypot(pos[0]-fp[0], pos[1]-fp[1], pos[2]-fp[2]);
              mipDistRef.current = actualDist > 10 ? actualDist : scale;
              window.__mipFP   = [...fp];
              window.__mipDist = mipDistRef.current;
              console.log('[MIP] setup lock — fp:', fp, 'dist:', mipDistRef.current, 'scale:', scale);
            }
          } catch(_e) {}
          // Start rotation after a short delay so the initial render completes first
          setTimeout(() => { if (!cancelled) setMipPlaying(true); }, 800);
        }
        if (cancelled) return;
        hasVP.current = true;
  ...
  useEffect(() => {
    ...
    function drawFrame() {
      ...
      try {
        const engine = getRenderingEngine(RENDERING_ENGINE_ID);
        const petVp = engine?.getViewport(viewportId);
        const ctVp  = engine?.getViewport(ctVpId);
        if (!petVp || !ctVp) return;
        const petSlice = getSlicePixelData(petVp);
        ...
        renderFusion(
          canvas,
          ctSlice.data,  ctSlice.width,  ctSlice.height,
          petSlice.data, petSlice.width, petSlice.height,
          glLUTRef.current,
          {
            ctLow:   ctWL_.wc  - ctWL_.ww  / 2,
            ctHigh:  ctWL_.wc  + ctWL_.ww  / 2,
            petLow:  petWL_.wc - petWL_.ww / 2,
            petHigh: petWL_.wc + petWL_.ww / 2,
            alpha: blend,
            power: 1.5,
          }
        );
      } catch(e) {
        console.warn('[canvasFusion] drawFrame:', e?.message);
      }
    }
  ...
  // Rebuild LUT when palette changes, trigger redraw
  useEffect(() => {
    if (modality !== 'PET' || !isVolume) return;
    glLUTRef.current = buildLUT(getColor, paletteId);
...
function CineBar({ viewportId, modality, isMIP, mipPlaying, onMipPlayingChange, mipFPRef, mipDistRef }) {
  ...
  const [fps, setFps]           = useState(isMIP ? 4 : 8)
  ...
  function _stepMIP(angleDelta) {
    ...
    try {
      ...
      const newPos = [
        fp[0] + dist * Math.sin(rad),
        fp[1] - dist * Math.cos(rad),
        fp[2],
      ]
      vp.setCamera({
        position:      newPos,
  ...
  return (
    <div style={{
      ...
      <input
        type="range"
        min={isMIP ? 1 : 1} max={isMIP ? 15 : 30} step={isMIP ? 1 : 1}
        value={fps}
